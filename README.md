...
手机扫码必须使用主机的 **局域网 IP**，不要使用手机自己的 `localhost`。当前 `npm start` 会自动检测一个局域网 IPv4，并把它写入二维码地址；启动日志会显示 `Phone QR host`。如果电脑有 VPN、多网卡或自动识别不对，可显式设置 `WAITING_PUBLIC_HOST=192.168.x.x` 后再启动。
...
大屏会提供 **扫码取号** 二维码，地址为 `:5177/queue`。顾客填写人数和称呼后获得 A001 / A002… 等位号，并看到前方桌数、当前顺位和等待时间。

服务员在 Host Console → **等位叫号** 中执行：
...
顾客取号页每约 1.2 秒读取自己的票号状态。到号时会显示全屏高亮，并在浏览器能力允许时播放声音、震动并发送系统通知。

> 当前部署是门店 LAN HTTP。手机完全关闭浏览器后，Web 页面无法保证后台系统通知；现场使用时建议让顾客保留取号页面或浏览器后台标签。大屏叫号 Overlay 同时作为兜底提醒。
